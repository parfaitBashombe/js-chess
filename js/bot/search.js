...
import { hashBoard, ttGet, ttGetMove, ttSet, TT_EXACT, TT_LOWER, TT_UPPER } from "./transposition.js";
...
export const setDeadline = (t) => { deadline = t; nodeCount = 0; timedOut = false; };
...
const hasNonPawnMaterial = (board, color) => {
  for (let r = 0; r < 8; r++)
    for (let c = 0; c < 8; c++) {
      const p = board[r][c];
      if (p && p.color === color && p.type !== "pawn" && p.type !== "king") return true;
    }
  return false;
};
...
const quiesce = (board, colorToMove, botColor, alpha, beta, enPassantTarget, ply = 0) => {
  ...
  for (const move of sorted) {
    ...
    const score = quiesce(next.board, oppositeColor(colorToMove), botColor, alpha, beta, next.enPassantTarget, ply + 1);
...
export const minimax = (board, colorToMove, botColor, depth, alpha, beta, enPassantTarget, isNullMove = false) => {
  ...
  const allMoves    = getAllLegalMovesForColor(board, colorToMove, enPassantTarget);
  ...
  if (depth === 0) return quiesce(board, colorToMove, botColor, alpha, beta, enPassantTarget);
  ...
  if (isMax && !isNullMove && !kingInCheck && depth >= 3 &&
      hasNonPawnMaterial(board, colorToMove) && !isEndgame(board)) {
    const R         = depth >= 6 ? 3 : 2;
    const nullScore = minimax(board, oppositeColor(colorToMove), botColor, depth - R - 1, alpha, beta, null, true);
    if (nullScore >= beta) return beta;
  }

  const ttMove    = ttGetMove(hash);
  const sorted    = sortMovesByPriority(board, allMoves, colorToMove, enPassantTarget, depth, ttMove);
  ...
  for (const move of sorted) {
    ...
    const next  = getNextBoardState(board, move, enPassantTarget);
    const score = minimax(next.board, oppositeColor(colorToMove), botColor, depth - 1, alpha, beta, next.enPassantTarget);
    ...
    if (beta <= alpha) {
      ...
      if (!move.to.capture && move.to.special !== "enPassant") addHistory(move, depth);
      break;
    }
  }
  ...
  let flag;
  if (isMax) {
    flag = bestScore <= alphaOrig ? TT_UPPER : bestScore >= betaOrig ? TT_LOWER : TT_EXACT;
  } else {
    flag = bestScore >= betaOrig  ? TT_LOWER : bestScore <= alphaOrig ? TT_UPPER : TT_EXACT;
  }
